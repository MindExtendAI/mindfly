...
import { walkingFoot } from './walking-kinematics.js';
...
export class Gait {
  constructor(model) {
    ...
    this.legs = LEGS.map((name, i) => {
      const segments = model.segments.filter(
        (s) => s === model.root || s.startsWith(name + '_'),
      );
      const local = {
        ...model,
        segments,
      ...
      const dofs = local.dofs.filter(
        (d) =>
          d.limitDeg &&
          (!d.child.includes('tarsus') || d.child.endsWith('tarsus1')),
      );
      return {
        name,
        i,
    ...
    for (let it = 0; it < iterations; it++) {
      ...
      if (
        ...
        (it > 0 ||
          ...
          leg.dofs.every(
            (d) =>
              Math.abs(this.pose[d.name] - this.referencePose[d.name]) < 0.001,
          ))
      )
        break;
      const eps = 0.001,
      ...
      for (let k = 0; k < 3; k++) {
        let p = k;
        for (let r = k + 1; r < 3; r++)
          if (Math.abs(mat[r][k]) > Math.abs(mat[p][k])) p = r;
      ...
      leg.dofs.forEach((d, j) => {
        ...
        this.pose[d.name] = Math.max(
          ...
          Math.min(
            hi,
            this.pose[d.name] + Math.max(-0.15, Math.min(0.15, delta)),
          ),
        );
      });
    }
  ...
  update(state) {
    ...
    for (const leg of this.legs) {
      ...
      const phase =
        (state.phase / (Math.PI * 2) + ([0, 2, 4].includes(leg.i) ? 0 : 0.5)) %
        1;
      const duty = 0.65,
        stride = Math.max(
          -1,
          Math.min(
            1,
            ((v - side * turn * Math.abs(leg.neutral[1])) / frequency) * duty,
          ),
        );
      ...
      let x =
        phase < duty
          ...
          : stride *
            (-0.5 + swing - Math.sin(swing * 2 * Math.PI) / (2 * Math.PI));
      let lift = Math.sin(swing * Math.PI) * 0.35 * Math.min(1, speed / 0.2);
      if (state.studyWalking) {
        const foot = walkingFoot(state.phase, v, leg.i);
        x = foot.x;
        lift = foot.lift;
      }
      ...
      if (state.grooming && [0, 3].includes(leg.i)) {
        // Illustrative foreleg rub, not joint tracking from the video.
        const rub = Math.sin(
          (state.time || 0) * Math.PI * 2 * 3 + (leg.i === 3 ? Math.PI : 0),
        );
        target[0] = 1.15 + 0.12 * rub;
        target[1] = side * (0.16 + 0.035 * rub);
        target[2] = 0.72 + 0.1 * Math.cos((state.time || 0) * Math.PI * 6);
      }
